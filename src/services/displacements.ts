...
const getDisplacementById = async (id: string): Promise<Displacement> => {
    const response: Promise<Displacement> = await fetchWrapper.get(`Deslocamento/${id}`)
...
const createDisplacement = async (displacement: Displacement): Promise<Displacement> => {
    const response: Promise<Displacement> = await fetchWrapper.post(`Deslocamento`, displacement)
